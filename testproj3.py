...
def get_height():
    while True:
        try:
            height_str = simpledialog.askstring("Input", "Please enter the pile height above the dredge line in meters")
...
def get_l4(a1, a2, a3, a4):
    ...
    real_roots = [root for root in roots if np.isreal(root) and not np.isnan(root) and np.isfinite(root)]
...
#Step 21: Perform a Sensitivity Analysis, by changing the shear strength parameter (phi) and comparing it to embedment depth and maximum moment
...
# Define a range of Phi values from 0 to 45 degrees with increments of 5 degrees
phi_values = np.arange(0, 47, 5)

# Initialize a list to store Mmax values for each Phi value
Mmax_values = []

# Iterate over each Phi value
for phi in phi_values:
    # Calculate the relevant parameters for the current Phi value
    Ka = get_ka(phi)
    Kp = get_kp(phi)
    sigma2 = get_sigma2(gamma_gransoil, height, Ka)
    l3 = get_l3(Kp, Ka, height)
    ...
    p = get_p(sigma2, height, l3)
    sigma5 = get_sigma5(gamma_gransoil, height, Kp, l3, Ka)
    l4 = get_l4(get_a1(gamma_gransoil, Kp, Ka, sigma5),
                get_a2(p, Kp, Ka, gamma_gransoil),
                get_a3(p, zbar, Kp, Ka, sigma5, gamma_gransoil),
                get_a4(p, zbar, sigma5, gamma_gransoil, Kp, Ka))
    zprime = get_zprime(p, Kp, Ka, gamma_gransoil)
    Mmax = get_Mmax(p, zbar, zprime, gamma_gransoil, Kp, Ka)
    
    # Append the calculated Mmax to the list
    Mmax_values.append(Mmax)

# Plot the results
plt.plot(phi_values, Mmax_values)
plt.xlabel('Phi (degrees)')
plt.ylabel('Maximum Moment (kN-m)')
plt.title('Effect of Phi on Maximum Moment')
plt.grid(True)
plt.show()
